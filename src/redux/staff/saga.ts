...
import { createStaff, deleteStaff, getStaff, getStaffDetails, updateStaff } from '../../api/StaffApi'; // Adjust the path as needed
import {
    getStaffSuccess,
    getStaffFailure,
    createStaffSuccess,
...
function* deleteStaffSaga(action: any): Generator<any, any, any> {
    try {
        const data = yield call(deleteStaff, action.payload.data, action.payload.id);
